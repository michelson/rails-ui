class RailsUi::ComponentPreview::Component < ViewComponent::Base
  ...
  def call
    content_tag :div, class: "border rounded-lg overflow-hidden my-2" do
      concat(content_tag(:h3, title, class: "bg-muted px-4 py-2 font-semibold"))
      concat(content_tag(:div, class: "flex flex-col md:flex-row") do
        concat(content_tag(:div, example, class: "bg-default preview flex min-h-[350px] w-full justify-center p-10 items-center border-b md:border-b-0 md:border-r"))
